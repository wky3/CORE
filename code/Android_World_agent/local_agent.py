...
from android_world.agents import local_agent_utils
from android_world.env import interface
from android_world.env import json_action
from android_world.env import representation_utils

# 与 m3a 大体一致，去掉原来 screenshot 元素，只保留 som
PROMPT_PREFIX = (
    ...
    ' At each step, you will be given the current screenshot with bounding'
...
ACTION_SELECTION_PROMPT_TEMPLATE = (
    ...
    'The current screenshot with bounding boxes and labels added'
...
def _generate_ui_elements_description_list(
  ...
  for index, ui_element in enumerate(ui_elements):
    if local_agent_utils.validate_ui_element(ui_element, screen_width_height_px):
...
class LocalAgent(base_agent.EnvironmentInteractingAgent):
  """LocalAgent which stands for Multimodal Autonomous Agent for Android."""
  ...
  def __init__(
    ...
    """Initializes a LocalAgent Agent.
  ...
  def step(self, goal: str) -> base_agent.AgentInteractionResult:
    ...
    print('----------step ' + str(len(self.history) + 1), flush=True)
    ...
    # 为 before_screenshot 添加 som
    before_screenshot = state.pixels.copy()
    for index, ui_element in enumerate(before_ui_elements):
      if local_agent_utils.validate_ui_element(ui_element, logical_screen_size):
        local_agent_utils.add_ui_element_mark(
            before_screenshot,
            ui_element,
            index,
            logical_screen_size,
    ...
    # 再存储一个原始的 before_screenshot_with_som 因为在最后会对 before_screenshot 加上 before 的水印
    # 但是 self-training 需要原始的 before_screenshot
    step_data['before_screenshot_with_som_raw'] = before_screenshot.copy()
    ...
    action_output, is_safe, raw_response = self.llm.predict_mm(
        action_prompt,
        [
            # step_data['raw_screenshot'],  不需要原始的 raw_screenshot
            before_screenshot,
        ],
    )
    ...
    if is_safe == False:  # pylint: disable=singleton-comparison
      ...
      action_output = f"""Action: {{"action_type": "status", "goal_status": "infeasible"}}"""
    ...
    action = local_agent_utils.parse_action_output(action_output)

    # If the output is not in the right format, add it to step summary which
    # will be passed to next step and return.
    if not action:
      print('Action prompt output is not in the correct format.', flush=True)
      step_data['summary'] = (
    ...
    try:
      ...
      print('Failed to convert the output to a valid action.', flush=True)
    ...
    if (
      ...
      if action_index >= num_ui_elements:
        print(
            ...
            f' UI element list only has {num_ui_elements} elements.', flush=True
        )
        step_data['summary'] = (
      ...
      # 这一步没必要，因为之后要进行迭代训练，需要真正的raw_screenshot，而不是经过标注的raw_screenshot
    ...
    if converted_action.action_type == 'status':
      if converted_action.goal_status == 'infeasible':
        print('Agent stopped since it thinks mission impossible.', flush=True)
    ...
    if converted_action.action_type == 'answer':
      print('Agent answered with: ' + converted_action.text, flush=True)

    def combine_action_and_observation(index, element_list):
      if index is not None:
          index = int(index)
          text = element_list[index].text
          content_description = element_list[index].content_description
          target_text = text or content_description
          return target_text
      return None

    try:
      # 先判断是否是 air mode
      target_text = combine_action_and_observation(action_index, before_ui_elements)
      if target_text != "Airplane mode":
        self.env.execute_action(converted_action)
      else:
        print("尝试开启 Airplane mode，不执行", flush=True)
    except Exception as e:  # pylint: disable=broad-exception-caught
      print('Failed to execute action.', flush=True)
      print(str(e), flush=True)
      step_data['summary'] = (
    ...
    # 没必要单独存储 after_screenshot
    # 为 after_screenshot 添加 som
    ...
    # 这里不需要加水印，因为没有summary的过程了，所以不需要，反而能够存储原始的 before_screenshot_with_som
    # 这里还单独为 before 和 after 加了水印
    # local_agent_utils.add_screenshot_label(
    #     step_data['before_screenshot_with_som'], 'before'
    # )
    # local_agent_utils.add_screenshot_label(after_screenshot, 'after')
    # step_data['after_screenshot_with_som'] = after_screenshot.copy()

    step_data['summary'] = f'Action selected: {action}'
    self.history.append(step_data)
    return base_agent.AgentInteractionResult(
        False,
        step_data,
