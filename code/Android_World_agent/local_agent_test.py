...
class LocalAgentInteractionTest(absltest.TestCase):
  ...
  def test_step_method_with_completion(self):
    ...
    agent = local_agent.LocalAgent(env, llm)
  ...
  def test_history_recording(self):
    env = test_utils.FakeAsyncEnv()
    # 这个本质是模拟回答，每调用一次 predict 就会按顺序输出相应的答案
    llm = MockMultimodalLlmWrapper([
        (
            (
    ...
    self.assertIn('fake answer', step1_data.data['summary'])
